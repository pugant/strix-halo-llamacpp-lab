...
# test-drafter-routing-t5.sh — T5 numerical spot-check (Task 10 of the T7-f2 plan — internal plan, not included in this repo — RS cycle 20/08)
# Gate G4 of the 2026-08-20-rs-rollback-dflash-experiment plan (internal plan, not included in this repo):
#   (a) MONO-MTP6 (route image)    ==  (b) DUAL+override mtp     -> char-identical
#   (c) DUAL+override dflash        vs  (d) DF7 ORIGINAL dflash2 image (RS off)
#   if (c)!=(d): (e) mono-DF7 on the NEW image (isolates the RS effect from routing).
# Fixed greedy prompt (plan Task 10): square root, max_tokens 400, temp 0.
...
IMG_ORIG=docker-llm-service:vulkan-fork-dflash2         # RS OFF (T7 reference)
PROMPT='Spiega in 3 righe cos e la radice quadrata, poi calcola quella di 144.'

boot() { # $1=name $2=img $3=type $4=nmax $5=drafter(or empty)
...
ask() { # $1 outfile $2 spec_drafter(or empty)
  local OV=""
  [ -n "$2" ] && OV="\"spec_drafter\":\"$2\","
  curl -s "http://127.0.0.1:${PORT}/v1/chat/completions" -H 'Content-Type: application/json' \
    -d "{$OV\"model\":\"q\",\"max_tokens\":400,\"temperature\":0,\"messages\":[{\"role\":\"user\",\"content\":\"$PROMPT\"}]}" > "$1"
}
arm() { # $1=letter $2=name $3=img $4=type $5=nmax $6=drafter $7=override
  echo "== T5 arm ($1) img=$3 type=$4 n-max=$5 override=${7:-none} =="
...
print('arm ($1): ct=%d finish=%s content_len=%d reason_len=%d' % (d['usage']['completion_tokens'], d['choices'][0]['finish_reason'], len(m.get('content') or ''), len(m.get('reasoning_content') or '')))"
  else
    echo "arm ($1): BOOT FAIL"; touch "$OUT/t5-rs-$1.FAIL"
  fi
  docker rm -f "$2" >/dev/null 2>&1
}
cmp_pair() { # $1,$2 letters
  python3 - "$OUT" "$1" "$2" <<'EOF'
import json,sys
def msg(a):
...
ci = a==b and c==d
print(f'({sys.argv[2]}) vs ({sys.argv[3]}): content_identical={a==b} reasoning_identical={c==d} -> {"CHAR-IDENTICAL" if ci else "DIVERGE"}')
EOF
}
...
# (e) conditional: only if (c)!=(d) — isolate the RS effect from routing
...
then
  ...
  echo "(e) not needed: (c) == (d)"
fi
echo "=== T5 END (artifacts t5-rs-*.json in logs/test-drafter-routing/) ==="
