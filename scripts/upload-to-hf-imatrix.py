...
# upload-to-hf-imatrix.py — publishes the Qwen3.8-27B imatrix (private → verify → public)
# User authorization: "for anyone who wants to use it" (session 19/08 evening).
import os, sys, time
...
if not expected.issubset(set(files)):
    log("[FAIL] missing files in tree"); sys.exit(1)

# flip to public (authorized)
try:
