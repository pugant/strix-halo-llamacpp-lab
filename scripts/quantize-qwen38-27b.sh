...
#   - SELF-PRODUCED imatrix (unsloth does not publish one for Qwen3.8) → run-imatrix-qwen38.sh
#   - NEVER stop/start llm-service (workspace rule: user manages it manually)
#   - unattended (autonomy)
#   - automatic dry-run before the real quantization
set -euo pipefail
...
NTHREADS=16
EXPECTED_OUT_MIN=13.5   # 27B × 4.38 bpw / 8 ≈ 14.8 GB; cautious range (deltanet can add)
...
docker_run() {  # $1=ro|rw, rest is the command
...
precheck() {
  ...
  [ "$avail_gb" -ge 40 ] && ok "Disk: ${avail_gb} GB free (≥40)" || { err "Disk: ${avail_gb} GB < 40"; exit 1; }

  docker ps --format '{{.Names}}' | grep -q '^llm-service$' \
    && warn "llm-service UP — CPU-only quantize can contend for UMA bandwidth (proceeding; rule: do not touch it)"
  ...
  [ $(( (s1+s2)/1024/1024/1024 )) -ge 50 ] && ok "BF16 complete (expected 50.9 GiB ($(( (s1+s2)/1024/1024/1024 )) GB))" || { err "BF16 incomplete"; exit 1; }

  [ -s "$IMATRIX_HOST" ] && ok "Imatrix present" || { err "Imatrix missing — run run-imatrix-qwen38.sh"; exit 1; }

  [ ! -f "$OUTPUT_HOST" ] && ok "Output target absent" || { err "$OUTPUT_HOST already exists — remove it first"; exit 1; }
}

dryrun() {
  log "Dry-run (expected size without quantizing)"
...
quantize() {
  ...
  log "Quantization (nthreads=${NTHREADS}, self-produced imatrix)"
  ...
  ok "Quantization completed in $((elapsed/60)) min $((elapsed%60)) sec"
  ...
  awk "BEGIN {exit !(${out_gb} >= ${EXPECTED_OUT_MIN} && ${out_gb} <= ${EXPECTED_OUT_MAX})}" \
    && ok "Size within the expected range [${EXPECTED_OUT_MIN}-${EXPECTED_OUT_MAX}] GB" \
    || warn "Size OUTSIDE the expected range — check"
}
...
case "$MODE" in
  ...
  *) err "Unknown mode: $MODE"; echo "Usage: $0 [--precheck|--dry-run|--quantize|--all]"; exit 1 ;;
esac
