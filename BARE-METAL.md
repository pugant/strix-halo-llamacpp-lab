...
| RAM | 128 GB unified LPDDR5X (~256 GB/s UMA bandwidth) — ~122 GB visible to the OS |
...
- Every measurement runs in a container of the tagged image named in the experiment's note (primary source: the note; the README engine table links to it).
...
OS/point-release history predates the lab's notes; kernel rows reflect package
install dates, not first boot.

## What we deliberately don't publish

LAN topology and internal addresses, hostname, serial numbers, local paths. We
