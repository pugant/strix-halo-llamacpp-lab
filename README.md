...
**Patch.** [`patches/qwen4exp-mtp/`](patches/qwen4exp-mtp/) — 10 commits, `git am`-clean (also included in [`rocmfpx/`](rocmfpx/) and on branch `qwen4exp-mtp`). Includes three silent-bug fixes worth knowing about if you port this yourself: the wide pre-norm width (`n_hc * n_embd`) for the drafter input, the PLE conv-state block missing from the checkpoint writer, and the mmproj×MTP abort (skip the embd-only image-chunk replay on the MTP draft context).
