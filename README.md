...
**Patch.** [`patches/qwen4exp-mtp/`](patches/qwen4exp-mtp/) — 11 commits, `git am`-clean (also included in [`rocmfpx/`](rocmfpx/) and on branch `qwen4exp-mtp`). Includes three silent-bug fixes worth knowing about if you port this yourself: the wide pre-norm width (`n_hc * n_embd`) for the drafter input, the PLE conv-state block missing from the checkpoint writer, and the mmproj×MTP abort (skip the embd-only image-chunk replay on the MTP draft context). Plus one line that turns on the RS ring-salvage rollback for qwen4exp (`llm_arch_supports_rs_rollback`): without it every partial verify rejection pays a full ~115-148 MiB checkpoint restore, which at long agent-style contexts turns into repeated `speculative replay stalled` drops of the drafter (measured 13 tok/s in production); with the ring the drafter never stalls — the known cost is a one-round acceptance dip right after a rollback (p0-reject 0.233 vs 0.071 baseline for that round only, mean accepted length 2.34→1.97 at greedy) and ~7.2 GiB of ring buffer.
