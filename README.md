...
- **Code** — the **full buildable source of the runtime fork is included in this repo under [`rocmfpx/`](rocmfpx/)** (snapshot of our fork at commit `62416acd3` — the routing stack, qwen4exp and the MTP draft head, plus the rollback-restore fix and the reasoning-budget warn window; a fork of [charlie12345/ROCmFPX](https://github.com/charlie12345/ROCmFPX), itself a fork of [ggml-org/llama.cpp](https://github.com/ggml-org/llama.cpp)). The same branches are also pushed here as git refs (`main`, `qwen4exp-rt`, `qwen4exp-mtp`, `qwen4exp-conv-ring-slots`), the work is carried as `git am`-clean patches, and [`docker/`](docker/) has the Dockerfiles to build the inference engine for both backends (HIP/ROCm 7.2.4 and Vulkan/RADV).
- **Models** (Hugging Face, weights under their own licenses):

| Repo | What it is |
|---|---|
| [`pugant/Qwen3.8-Flash-Next-Q4_0_ROCMFP4_STRIX_LEAN-GGUF`](https://huggingface.co/pugant/Qwen3.8-Flash-Next-Q4_0_ROCMFP4_STRIX_LEAN-GGUF) | **qwen4exp 180B (6B active + 51B PLE), 98.5 GiB** — the lab's largest model; pairs with an external MTP drafter for 2×+ deterministic decode (drafter requires the fixed runtime — see the qwen4exp section) |
...
**What.** The full `qwen4exp` architecture on the fork — gated-deltanet linear attention, QSA indexer attention with a third KV stream, the 51B-parameter PLE n-gram table, 4-stream low-rank hyper-connections — plus the **MTP/NextN draft head driven by an external drafter GGUF** (`-md`), ported from upstream [PR #27836](https://github.com/ggml-org/llama.cpp/pull/27836) and adapted to the fork (converter and quantizer side from [PR #27742](https://github.com/ggml-org/llama.cpp/pull/27742)). Runs unchanged on HIP and Vulkan builds. Vision (`--mmproj`) works **together with** the MTP drafter: an image chunk is embd-only while the MTP draft graph requires token input, so the drafter-side replay is skipped — the head still sees the image through the trunk hidden state at draft time, and the target's verify keeps the output exact (2026-08-28 fix; upstream PR #27836 has the same latent abort).
...
Draft acceptance 95.7% of tokens (mean accepted length 3.24 at n=3; still 5.20/6 at n=5 on deterministic text). After the mmproj×MTP fix: acceptance on a vision request measured 98.3% cumulative — the drafter-side image skip is effectively free. Perplexity sanity identical to the pre-MTP build. The mechanical read: the round bottleneck is the **batched verify over the hybrid trunk** (KV + GDN + QSA index + PLE), not the drafter — deterministic work converts acceptance into speed almost 1:1, open prose decays after position 1. **After the rollback-restore fix (patch 0012, 2026-08-30) the same stack on Vulkan/RADV sustains 41-44 tok/s at n=6 on code-generation workloads (draft acceptance 0.91-0.95) — ~+70% over the pre-fix server — and n=6 is the measured optimum (n=8 regresses to 35 tok/s: draft positions 7-8 do not pay for themselves).** Model card with the full numbers: [Qwen3.8-Flash-Next-Q4_0_ROCMFP4_STRIX_LEAN-GGUF](https://huggingface.co/pugant/Qwen3.8-Flash-Next-Q4_0_ROCMFP4_STRIX_LEAN-GGUF).

**Patch.** [`patches/qwen4exp-mtp/`](patches/qwen4exp-mtp/) — 20 commits, `git am`-clean (also included in [`rocmfpx/`](rocmfpx/) and on branch `qwen4exp-conv-ring-slots`). Includes four silent-bug fixes worth knowing about if you port this yourself: the wide pre-norm width (`n_hc * n_embd`) for the drafter input, the PLE conv-state block missing from the checkpoint writer, the mmproj×MTP abort (skip the embd-only image-chunk replay on the MTP draft context), and — the one that matters most in practice — **the conv/PLE ring-slot writer**: `build_conv_state_at` wrote only ring group 0, so a partial-reject rollback restored the GDN conv history and the PLE history from never-written (zeroed) slots while the SSM rewound correctly; every rollback round silently degraded the output (lost digits inside hex/number runs, spliced fragments, premature stops), which is easy to misread as a quantization problem — it is not. Plus one line that turns on the RS ring-salvage rollback for qwen4exp (`llm_arch_supports_rs_rollback`): without it every partial verify rejection pays a full ~115-148 MiB checkpoint restore, which at long agent-style contexts turns into repeated `speculative replay stalled` drops of the drafter (measured 13 tok/s in production); with the ring the drafter never stalls. What we had first measured as the ring's inherent cost — a one-round acceptance dip right after a rollback (p0-reject 0.233 vs 0.071, mean accepted length 2.34→1.97 at greedy) — was in fact the symptom of that missing writer: with the fix the dip disappears (post-rollback target entropy converges to baseline), acceptance rises from ~0.74 to 0.91-0.95, and decode speed gains ~+70% at n=6. The ring itself costs ~7.2 GiB of buffer. The same series also carries the **reasoning-budget warn window** (patches 0016-0020, full note in [`docs/research/reasoning-budget-warn75.md`](docs/research/reasoning-budget-warn75.md)): at 75% of the thinking budget a mid-conversation message asks the model to converge, with a residual window before the forced end — in a real one-hour agent session (48 requests, budget 17408) every request closed naturally, the single injected warning converged in 1.3 s, and no budget was exhausted.

**Build the engine.** [`docker/Dockerfile.rocm-7.2.4-rocmfpx`](docker/Dockerfile.rocm-7.2.4-rocmfpx) (HIP, gfx1151) or [`docker/Dockerfile.vulkan-rocmfpx`](docker/Dockerfile.vulkan-rocmfpx) (Vulkan/RADV), both `--build-arg REPO=<this-repo>.git --build-arg BRANCH=qwen4exp-conv-ring-slots` (any branch that includes patch 0012 — 0016+ if you want the reasoning-budget warn window; using an older build **with** the MTP drafter means corrupted output after rollback rounds). For builds without GitHub access to the private fork mirror, [`docker/Dockerfile.vulkan-rocmfpx-local`](docker/Dockerfile.vulkan-rocmfpx-local) builds the same image from a plain `git archive` context. Remember: HIP containers need `/dev/kfd` **and** `/dev/dri` — without `/dev/kfd` ROCm init fails *silently* and the server falls back to CPU.
