...
- **Code** — the **full buildable source of the runtime fork is included in this repo under [`rocmfpx/`](rocmfpx/)** (snapshot of our fork at commit `c1d31353e` — the routing stack, qwen4exp and the MTP draft head, plus the rollback-restore fix; a fork of [charlie12345/ROCmFPX](https://github.com/charlie12345/ROCmFPX), itself a fork of [ggml-org/llama.cpp](https://github.com/ggml-org/llama.cpp)). The same branches are also pushed here as git refs (`main`, `qwen4exp-rt`, `qwen4exp-mtp`, `qwen4exp-conv-ring-slots`), the work is carried as `git am`-clean patches, and [`docker/`](docker/) has the Dockerfiles to build the inference engine for both backends (HIP/ROCm 7.2.4 and Vulkan/RADV).
