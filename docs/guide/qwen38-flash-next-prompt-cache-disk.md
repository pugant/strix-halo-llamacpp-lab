...
Deployed on a real always-on multimodal agent server since **2026-09-01**; rolling back is removing the flags and restarting (see [Rollback](#rollback)).

## What it is

The fork's server already keeps an SSD-backed prompt cache (`--cache-disk`) for mid-session reuse — but its metadata lives in RAM and is wiped at boot, so every restart starts from an empty cache. `--cache-disk-persist` adds a **second, durable tier**: entries are promoted to a library under `<cache-dir>/.llama-prompt-cache-v1/persist/entry-<id>/` — a saved state payload for the target (plus the drafter's, when speculative decoding is on) and a sidecar with an 88-byte little-endian header (configuration fingerprint, hit count, timestamps, CRC-32s) followed by the token-id array.
...
Whether your client can speak verbatim is a property of the client. Also note that a saved entry includes the generated reply, so a bare resend of the prompt alone is a strict prefix of the entry — it takes the re-processing path rather than a clean restore; the intended use is **extending** a restored context with a new turn.
...
Boot fails fast if `--cache-disk-persist` is set without a `--cache-disk` path or with the per-run limit (`--cache-disk-limit`, default 8192 MiB) disabled. The flags compose with `--ple-disk` and `-md <drafter>` — that combination is the production configuration. Pair the directory with real disk (NVMe): a ~107k-token entry is ~2.1 GiB on disk (the saved state is dominated by the fixed-size recurrent state plus per-position KV, so a short prompt still pays most of a fixed floor — the 1,449-token production entry is 133 MB).

## What to expect

- **Restore**: the state load for a 107,283-token entry measured **1.28–1.57 s** (end-to-end 14.3 s vs 920 s cold: **64×**). Each restore additionally pays one full CRC read of the payload, which the kernel absorbs once.
...
- **RAM: unchanged.** This is a latency feature; the entry lives on SSD within its own budget. (The RAM relief on this model comes from `--ple-disk`, which composes with this.)
- **Determinism**: the same library rebuilt in an independent boot re-served the same request with char-identical output (111/111 characters) — the restore is deterministic across restarts.

## Monitoring

Twelve `llamacpp:persist_*` counters and gauges on `/metrics` (the endpoint requires `--metrics`, otherwise the server returns 501); the main ones:
...
The quick health check is the boot line in the logs — `persist boot adopt: entries=N orphans=N gc=N bytes=N budget_bytes=N` — and, after a request you expect to be restored, `persist load: entry=N restored=R prefilled=P ms=M`: **R is the number that tells you the restore fired** (P alone can hide a silent full prefill).
...
**Does it work with vision/multimodal prompts?** Yes, in production since 2026-09-01 (the first deploy iteration had three save-path bugs on multimodal slots, all fixed at the root cause: the save path now keys on a prompt's actual media cells; a multimodal prompt saves its text-token view).
...
Remove `--cache-disk-persist` (and `--cache-disk` if it was added for it) and restart: the library on disk is left alone — never read, never written — and the server behaves exactly as before the feature existed. No data migration in either direction; deleting the `persist/` subtree under the cache dir reclaims the space.
