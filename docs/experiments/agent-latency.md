...
llama.cpp fork; the target was Qwen3.8-27B-Q4_0_ROCMFP4_STRIX_LEAN (the lab's dense
hybrid of that week). Production serving flags: dual-drafter routing (the target's
built-in MTP head for prose, the DFlash2 block-diffusion drafter for tool calls),
KV q8_0, context to 262k; a long-horizon thinking agent with tool calls as the load,
temp 0 throughout. Instrumentation: a tee-proxy timestamping every stream chunk, a
2 s hardware sampler, the server log — every number below traces to a log line or an
analysis key, cross-checked by a second reviewer before the reports were closed.
...
- **Fix validated on a real 6 h 45 m session: 0 cold fallbacks** (was 7 in 36),
  ...
  plus an end-of-session mega-prefill that is apparently client-side history
  compaction, due by design and not cacheable.
...
with `reason=target-draft-restore-rejected`, six of the seven following an aborted
request (the seventh followed a normally-completed turn) — hence the first reading:
*abort invalidates the slot, the next request re-evaluates everything*.
