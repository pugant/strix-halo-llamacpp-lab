...
struct common_params_speculative_draft {
    ...
    // dual mode: dedicated MTP context (on the target model), while ctx_dft stays
    // bound to the external draft model (e.g. DFlash). Null in legacy mono-MTP mode,
    // where the MTP implementation uses ctx_dft.
...
struct common_params_speculative {
    ...
    // against this JSON fixtures file (generated by the certified Python
    // reference: scripts/t8-b2-detector.py --dump-fixtures) and exit - before
    // any backend init, model load or inference. Exit code 0 = PASS n/n,
    // 1 = FAIL or unreadable fixtures.
    std::string concat_selftest;
