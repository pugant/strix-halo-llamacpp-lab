...
void llama_model_saver::add_kv_from_model() {
    ...
    if (model->arch == LLM_ARCH_QWEN4EXP) {
        // one compress ratio per layer, exactly as the loader expects them
        const std::vector<uint32_t> compress_ratios(
                hparams.attn_compress_ratio.begin(), hparams.attn_compress_ratio.begin() + hparams.n_layer);
...
void llama_model_saver::add_tensors_from_model() {
    ...
    add_tensor(model->output_hc_fn);
    add_tensor(model->output_hc_base);
    add_tensor(model->output_hc_scale);
    add_tensor(model->per_layer_tok_embd);
    add_tensor(model->output_hc_norm);
    add_tensor(model->output_hc_down);
    add_tensor(model->output_hc_up);
