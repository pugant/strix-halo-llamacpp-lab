...
# Qwen3.8-Flash-Next (qwen4exp): ported from upstream PR ggml-org/llama.cpp#27742 (danielhanchen).
...
class Qwen4ExpTextModel(_Qwen35MRopeMixin, _LinearAttentionVReorderBase):
    ...
    def _read_hash_constants(self, suffix: str) -> list[int]:
        ...
        The generic tensor pipeline casts every non-float dtype to float32 before
        modify_tensors() sees it, which would silently round these 45-bit
        multipliers. Reading the lazy tensor here bypasses that.
        """
        for name, gen in self.model_tensors.items():
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        # int64 hash constants must stay exact; 1-D tensors force F32, so keep them aside
    ...
    # -- the PLE table ----------------------------------------------------
    #
    # The 128 shards concatenate into one enormous tensor, which would otherwise peak
    # near 300 GB of RSS. Each shard is written straight into a memory-mapped file at
    # its final row offset and then dropped, so only one shard is resident. The file
    # is removed after the write. It holds float32 because the pipeline has already
    # cast the shards to it.
